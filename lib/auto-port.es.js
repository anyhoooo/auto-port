...
var GenerateFunction = /*#__PURE__*/function () {
  ...
  _createClass(GenerateFunction, [{
    key: "apiTemplate",
    value: function apiTemplate(api, usedModel, usedEnum, moduleName) {
...
function transCSharpTypeToTyscriptType(origintype) {
  var typeEnmu = {
    integer: 'number',
    string: 'string',
    "boolean": 'boolean',
...
function getTyscriptType(target, definitions) {
  ...
  if (target.$ref) {
    var type = getModeleType(target.$ref); // let {
    // 	name,
    // 	isArray
    ...
    if (target.oneOf) {
      ...
      if (target.type === "array") {
        if (target.items.$ref) {
          var _type = getModeleType(target.items.$ref);

          !definitions.includes(_type) && definitions.push(_type);
          return "".concat(_type, "[]");
        } else {
          return "".concat(transCSharpTypeToTyscriptType(target.items.type), "[]");
        }
      } else {
    ...
    if (used.includes(key)) {
      ...
      if (!item.enumname) {
        if (_typeof(item["enum"][0]) === 'object') {
          Object.keys(item["enum"][0]).forEach(function (key) {
            context += "".concat(key, " = ").concat(item["enum"][0][key], ",");
          });
        } else {
          console.log("\u8BA9\u540E\u7AEF\u628A".concat(key, "\u8FD9\u4E2A\u679A\u4E3E\u52A0\u4E0A\u63CF\u8FF0"));
          item["enum"].forEach(function (key, index) {
            context += "Enum".concat(index, " = ").concat(item["enum"][index], ",");
          });
        }
      } else {
        Object.keys(item.enumname).forEach(function (key) {
          context += "".concat(key, " = ").concat(item.enumname[key], ",");
        });
      }

      var template = "\n\t\t\t\texport enum ".concat(item.name, " {\n\t\t\t\t\t").concat(context, "\n\t\t\t\t}");

      if (checkIsNeedUpdate('enum', module + '/' + key, item)) {
        writeFile(key, template, dirname); // console.log(chalk.greenBright(dirname + '/' + key + ' 枚举已更新'))
      }
  ...
  Object.keys(modelMap).forEach(function (key) {
    if (used.includes(key)) {
      ...
      item.property.forEach(function (key, i) {
        context += "\n\t\t\t\t\t/**  ".concat(key.description, " **/\n\t\t\t\t\t").concat(key.name).concat(key.required ? '' : '?', ": ").concat(key.type, "\n\t\t\t\t\t");
      });
      var template = "\n\t\t\t\t".concat(usedModel.map(function (i) {
        return i === key ? ' ' : "import { ".concat(i, " } from './").concat(i, "'");
      }).join('\r\n'), "\n\t\t\t\t").concat(usedEnum.map(function (i) {
        return "import { ".concat(i, " } from '../Enum/").concat(i, "'");
      }).join('\r\n'), "\n\n\t\t\t\t/**  ").concat(item.description, " **/\n\t\t\t\texport interface ").concat(item.name).concat(item["extends"] ? ' extends ' + item["extends"].join(',') : '', " {\n\t\t\t\t\t").concat(context, "\n\t\t\t\t}");

      if (checkIsNeedUpdate('type', module + '/' + key, item)) {
        writeFile(key, template, dirname); // console.log(chalk.greenBright(dirname + '/' + key + ' 模型已更新'))
      }
...
function apiFile(module, apis, enumMap, modelMap, dirname) {
  var template = function template(apiList) {
    return "\n\t\t/**\n\t\t* @description Tag \u63A5\u53E3\u6C47\u603B\n\t\t*/\n\t\t".concat(apiList.map(function (i) {
      return "import { ".concat(i, " } from './").concat(i, "'");
    }).join('\r\n'), "\n\n\t\texport default { ").concat(apiList.join(' , '), " }\n\t\t");
  };
  ...
  apis.forEach(function (api) {
    if (checkIsNeedUpdate('api', module + '/' + api.name, api)) {
      ...
      writeFile(api.name, generateFun.apiTemplate(api, usedModel, usedEnum, module), dirname); // console.log(chalk.greenBright(dirname + '/' + api.name + ' 接口已更新'))
    }
  ...
  if (checkIsNeedUpdate('tag', module, apiList.join(' , '))) {
    writeFile('index', template(apiList), dirname); // console.log(chalk.yellowBright(module + '控制器已更新'))
  } //Api 改名相当于新建一个，删除无用的
  //删除没有的Api
...
function readLocalFile() {
  ...
  var count = 0;
  var filelist = ['../mock/swagger.mock3.json', '../mock/swagger.mock2.json'];
  checkOutputDirExit(apiConfig$3.outputDir);
  filelist.forEach(function (url) {
    fs$2.readFile(path$2.resolve(__dirname, url), 'utf-8', function (err, data) {
      if (err) {
        ...
        setTimeout(function () {
          ...
          compile(res, apiConfig$3.outputDir);
...
function compile(swaggerjson) {
  ...
  Object.keys(pathMap).forEach(function (module) {
    ...
    apiFile(module, moduleApis, enumMap, modelMap, moduleDir + '/apis');
  });
}
