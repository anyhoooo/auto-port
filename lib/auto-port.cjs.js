...
function getModeleType(str) {
  return str.replace('#/definitions/', ''); // return str.match(/(\w+)$/g)[0]
}
/** 写入文件 */
...
function parseProperties(target, definitions) {
  ...
  Object.keys(target.properties).map(function (pro) {
    ...
    propetys.push(_objectSpread2(_objectSpread2({}, targetPro), {}, {
      name: pro,
...
function paths(paths) {
  ...
  Object.keys(paths).forEach(function (api) {
    var name = api.split('/');
    var method = Object.keys(paths[api])[0];
    var info = {
      name: new apiConfig$1.GenerateClass().nameRule ? new apiConfig$1.GenerateClass().nameRule(api) : name[name.length - 2] + name[name.length - 1],
      method: method,
      url: api,
      request: [],
      summary: ''
    ...
    if (data.tags) {
      tag = new apiConfig$1.GenerateClass().getTagName ? new apiConfig$1.GenerateClass().getTagName(api) : getTagName(data.tags);
    } else {
      console.log('该Api没有标识Tag，默认放到Other里');
      tag = 'Other';
    }

    if (!(apiConfig$1.excludeModule && apiConfig$1.excludeModule.includes(tag))) {
      if (!pathMap[tag]) {
        pathMap[tag] = [];
      }
...
var conflictName = ['function', 'void', 'number', 'string', 'int'];
/** 处理接口入参 */
  ...
  parameters.map(function (param) {
    if (param["in"] === 'query' || param["in"] === 'body') {
      if (conflictName.includes(param.name)) {
        argument.push(_objectSpread2(_objectSpread2({}, param), {}, {
          name: param.name,
          conflictName: "_".concat(param.name),
  ...
  Object.keys(modelMap).forEach(function (key) {
    if (used.includes(key)) {
      ...
      item.property.forEach(function (key, i) {
        ...
        context += "\n\t\t\t\t\t/**  ".concat(key.description, " **/\n\t\t\t\t\t").concat(key.name).concat(apiConfig$2.isIgnoreRequired ? '' : key.required ? '' : '?', ": ").concat(key["enum"] ? "'".concat(key["enum"].join("'|'"), "'") : generateFun.aliasModelName(key.type), "\n\t\t\t\t\t");
      });
      var template = "\n\t\t\t\t".concat(usedModel.map(function (i) {
...
function compile(swaggerjson) {
  ...
  if (typeof swaggerjson === 'string') {
    var source = swaggerjson.replace(/\n/gm, ' '); // 下面校验给定字符串是否为一个合法的json
