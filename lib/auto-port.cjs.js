  ...
  if (target.$ref) {
    ...
    if (target.oneOf) {
      ...
      if (data.type === "array") {
        ...
        if (data.allOf) {
          var _type2 = getModeleType(data.allOf[0].$ref);
          ...
          if (innerData.type === "array") {
            if (innerData.items.$ref) {
              ...
              return "".concat(_type2, "<").concat(innerType, "[]>");
            } else {
              return "".concat(_type2, "<").concat(transCSharpTypeToTyscriptType(innerData.items.type, innerData.items.format), "[]>");
            }
          } else {
            if (innerData.$ref) {
              ...
              return "".concat(_type2, "<").concat(_innerType, ">");
            } else {
          ...
          var _type3 = getModeleType(data.$ref);
      ...
      if (target.type === "array") {
        if (target.items.$ref) {
          var _type4 = getModeleType(target.items.$ref);

          !definitions.includes(_type4) && definitions.push(_type4);
          return "".concat(_type4, "[]");
        } else {
...
function paths(paths) {
  ...
  Object.keys(paths).forEach(function (api) {
    var name = api.split('/');
    var info = {
      name: apiConfig$1.version === 'V2' ? name[name.length - 2] + name[name.length - 1] : name[name.length - 1],
