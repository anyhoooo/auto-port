...
function getTyscriptType(target, definitions) {
  ...
  if (target.$ref) {
    var type = getModeleType(target.$ref);
    ...
    if (target.oneOf) {
      ...
      if (data.type === "array") {
        if (data.items.$ref) {
          var _type = getModeleType(data.items.$ref);

          console.log(_type);
          !definitions.includes(_type) && definitions.push(_type);
          return "".concat(_type, "[]");
        } else {
          return "".concat(transCSharpTypeToTyscriptType(data.items.type), "[]");
        }
      } else {
        if (data.$ref) {
          var _type2 = getModeleType(data.$ref);
      ...
      if (target.type === "array") {
        if (target.items.$ref) {
          var _type3 = getModeleType(target.items.$ref);
